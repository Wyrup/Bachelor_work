...
BASE_DIR="generated_contracts/llama3.1-8b_temp_0_7"
...
DEFAULT_LLM_CSV="$BASE_DIR/analysis/llm/ollama_results_llama3.1-8b.csv"
...
for PROMPT_NAME in "${PROMPTS[@]}"; do
  ...
  TABLES_DIR="$BASE_DIR/analysis/thesis_tables_${PROMPT_NAME}"
  OLLAMA_CSV="$LLM_DIR/ollama_results_llama3.1-8b.csv"
  MASTER_CSV="$MASTER_DIR/master_dataset.csv"

  mkdir -p "$LLM_DIR" "$MASTER_DIR" "$STATS_DIR" "$TABLES_DIR"
  ...
  python3 Code/Solidity_Generation/thesis_master_tables.py \
    "$MASTER_CSV" \
...
printf '\nAll llama3.1:8b prompt runs completed.\n'
