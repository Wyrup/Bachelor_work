...
def render_progress(done, total, start_time, active_files):
    ...
    return f"[{bar}] {done}/{total} ({pct:5.1f}%) | elapsed: {format_duration(elapsed)} | running: {active}"


def analyze_file(label, path, timeout, myth_cmd, done_counter, total, start_time, active_set, active_lock):
    ...
    with _print_lock:
        with active_lock:
            print(render_progress(done_counter[0], total, start_time, sorted(active_set)))
        print(
            f"  -> done: {name} | myth_status={result['status']} | "
...
def main():
    ap = argparse.ArgumentParser(
        description="Run Mythril on SAFE Solidity files in parallel",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples
--------
  python3 select_run_safe.py /data/vuln-root --mode random --n 50 --seed 42 --workers 4
  python3 select_run_safe.py /data/vuln-root --mode smallest --n 100 --workers 4
  python3 select_run_safe.py /data/vuln-root --mode range --start 1 --end 100 --workers 4
""",
    )

    ap.add_argument("root", help="Root folder containing the SAFE sub-folder")
    ap.add_argument("--mode", choices=["random", "smallest", "range"], default="random", help="File selection mode")
    ap.add_argument("--n", type=int, default=None, help="Files per folder for random/smallest")
    ap.add_argument("--start", type=int, default=None, help="1-based start index for range mode")
    ap.add_argument("--end", type=int, default=None, help="1-based end index for range mode")
    ap.add_argument("--timeout", type=int, default=120, help="Mythril timeout per file in seconds")
    ap.add_argument("--workers", type=int, default=2, help="Parallel Mythril processes")
    ap.add_argument("--myth", default="myth", help="Mythril executable")
    ap.add_argument("--outdir", default="results_safe", help="Base output directory")
    ...
    picked, missing = choose_files(args.root, n=args.n, mode=args.mode, batch_start=args.start, batch_end=args.end)
    ...
    total = len(picked)
    sel_info = f"Mode=range ({args.start}-{args.end})" if args.mode == "range" else f"Mode={args.mode} | n={args.n}"
    print(f"\n{sel_info} | {total} SAFE files total | workers={args.workers} | output -> {outdir}")
    for _, f in picked[:10]:
        print(f"  [SAFE] {f.name}")
    if total > 10:
    ...
    print(f"\n[########################] {total}/{total} (100.0%) | elapsed: {format_duration(elapsed_total)} | done\n")
